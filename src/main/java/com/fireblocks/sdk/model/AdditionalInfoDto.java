  ...
  **/
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getEstimatedAnnualReward() != null) {
      joiner.add(String.format("%sestimatedAnnualReward%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getEstimatedAnnualReward()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `lockupPeriod` to the URL query string
    if (getLockupPeriod() != null) {
      joiner.add(String.format("%slockupPeriod%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getLockupPeriod()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `activationPeriod` to the URL query string
    if (getActivationPeriod() != null) {
      joiner.add(String.format("%sactivationPeriod%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getActivationPeriod()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
