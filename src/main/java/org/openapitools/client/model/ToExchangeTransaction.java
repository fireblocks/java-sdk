  ...
  **/
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getDstAddress() != null) {
      try {
        joiner.add(String.format("%sdstAddress%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getDstAddress()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getDstTag() != null) {
      try {
        joiner.add(String.format("%sdstTag%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getDstTag()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
