  ...
  **/
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getContractAddress() != null) {
      try {
        joiner.add(String.format("%scontractAddress%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getContractAddress()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getNativeAsset() != null) {
      try {
        joiner.add(String.format("%snativeAsset%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getNativeAsset()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getDecimals() != null) {
      try {
        joiner.add(String.format("%sdecimals%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getDecimals()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
