...
public class AuthorizationGroups {
  ...
  **/
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getTh() != null) {
      try {
        joiner.add(String.format("%sth%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getTh()), "UTF-8").replaceAll("\\+", "%20")));
      ...
      for (String _key : getUsers().keySet()) {
        try {
          joiner.add(String.format("%susers%s%s=%s", prefix, suffix,
              ...
              getUsers().get(_key), URLEncoder.encode(String.valueOf(getUsers().get(_key)), "UTF-8").replaceAll("\\+", "%20")));
        } catch (UnsupportedEncodingException e) {
          // Should never happen, UTF-8 is always supported
          throw new RuntimeException(e);
        }
